...
import org.byrde.akka.http.logging.impl.{ErrorHttpLogger, RequestHttpLogger}
...
class ModuleBindings extends AbstractModule with ScalaModule with AssistedInjectFactoryScalaModule[Binder] {
  override def configure(): Unit = {
    ...
    bind[HttpRequestLogging].to[RequestHttpLogger].asEagerSingleton()
    bind[HttpErrorLogging].to[ErrorHttpLogger].asEagerSingleton()
